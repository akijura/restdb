...
func ListenAllUsers() []User {
	...
	for rows.Next() {
		err = rows.Scan(&c1, &c2, &c3, &c4, &c5, &c6)
		if err != nil {
			log.Println("ListAllUsers Scan:", err)
			return all
		}
...
// IsUserAdmin determines whether a user is admin or not
...
// FindUserUsername if for returning a user record by a username
